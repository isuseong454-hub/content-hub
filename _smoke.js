...
(async function smoke() {
  ...
  // ── 10. 2026-08-20 리디자인 항목 (유형 4종·라임·유리 도크·작업실 톤) ──
  {
    ...
    if (ut) {
      ...
      /* 🚨 2026-08-21 해다링 원고(진단글)로 확인하다 잡은 것들 */
      add('사람이 쓰는 목록 기호', et.indexOf('✔✅☑✓') >= 0, '✔ ✅ 가 빠져 체크리스트가 통째로 문단이 됐다');
      add('년·월·일 시:분 날짜', et.indexOf("\\d{0,2}\\s*\\.?\\s*(\\d{1,2}:\\d{2})?") >= 0, '「2026. 8. 20. 12:17」을 못 걸러냈다');
      ...
      /* 🚨🚨 2026-08-21 사장님 «자동 로그인인데 로그아웃도 운영자 모드도 못 간다» — 갇혔다.
            ...
            🚨 갇히는 화면은 절대 만들면 안 된다. 이 항목이 빨간 줄이면 반드시 멈춘다. */
      /* 🚨 이 항목의 첫 판은 «화면에 찍힌 글자»를 찾았는데, 그 글자는 코드에서
        '...data-lm="'+it[0]+'"...' 로 «조립»된다 — 소스엔 그 모양으로 없다.
        조립되는 «재료»를 본다. */
      add('현장에 나가는 문', et.indexOf('id="le-more"') >= 0 && et.indexOf("['out','\uD83D\uDEAA','로그아웃'") >= 0, '자동 로그인이면 로그아웃 없이는 영영 갇힌다');
      ...
      /* 🚨 2026-08-21 사장님 «로그인하면 잠깐 예전 홈이 떴다가 지금 홈이 뜬다» —
            착지를 고정 시간(520·620ms)에 걸어 둬서 서버 데이터가 오기 «전»에 튀어 나갔다.
            그 틈에 로컬에 남은 옛 홈이 보였다. 시간이 아니라 «데이터 준비»를 기다려야 한다. */
      add('착지는 데이터 온 뒤', et.indexOf('var _ready = Promise.race') >= 0
      ...
      /* 🚨 2026-08-21 — 비교 미리보기가 가짜 글(__pv_…)을 열며 조회수를 쌓아
            test22 «인기» 상위 5개가 전부 시험 찌꺼기였다. 시험이 통계를 오염시키면 안 된다. */
      add('미리보기는 추적 안 함', ut.indexOf("/^__pv_|^__(rt|bar|preview)/.test(_lb)") >= 0,
